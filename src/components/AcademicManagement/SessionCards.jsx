...
const SessionCards = ({ classes = [] }) => {
  ...
  const getSessionStatus = (clase, index) => {
  ...
  return (
    ...
    >
      {classes.map((clase, index) => (
        <div 
          key={index} 
          className={`relative bg-white/5 border ${activeSession === clase.class_id ? 'border-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.1)]' : 'border-white/10'} rounded-2xl p-7 transition-all duration-300`}
        >
          {/* Badge de Estado */}
          <div className="absolute top-6 right-7 flex items-center gap-2.5 bg-white/5 px-3 py-1.5 rounded-full border border-white/10">
            <span className={`h-2.5 w-2.5 rounded-full ${getStatusColor(clase)}`}></span>
            <span className="text-[10px] font-bold uppercase tracking-wider text-gray-300">
              {getSessionStatus(clase, index)}
            </span>
          </div>
          ...
          <div className="space-y-6 mt-6">
            <div>
                ...
                <p className="text-2xl font-bold text-blue-100">{clase.subject}</p>
            </div>
            ...
            <div className="bg-white/5 p-4 rounded-xl border border-white/10 flex items-center justify-between">
                ...
                <div className="text-right">
                    <p className="text-[9px] text-gray-500 font-black uppercase tracking-widest">Tiempo Restante</p>
          ...
          {activeSession === clase.class_id && !isMinimized && (
            ...
            >
                ...
                <div className="flex justify-between items-start mb-6">
                    <div>
                        <h3 className="text-xl font-bold text-white tracking-tight">Gestión: {clase.subject}</h3>
                        <p className="text-[10px] text-gray-500 font-mono">Control de Sesión Activa</p>
                    </div>
                    <button 
                ...
                <p className="text-center text-[9px] text-gray-600 mt-4 uppercase tracking-[2px]">CatsiVard • Secure Session</p>
            </div>
          )}
        </div>
      ))}
